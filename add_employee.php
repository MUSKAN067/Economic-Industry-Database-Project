...
if(isset($_POST['submit'])){
	...
	if($id>0){
		$sql="update employee set name='$name',email='$email',mobile='$mobile',password='$password',
		department_id='$department_id',role='$role_id',address='$address',birthday='$birthday' where id='$id'";
	}else{
		$sql="insert into employee(name,email,mobile,password,department_id,address,birthday,role) 
		values('$name','$email','$mobile','$password','$department_id','$address','$birthday','$role_id')";
	}
...
<div class="content pb-0">
            <div class="animated fadeIn">
               <div class="row">
                  <div class="col-lg-12">
                     <div class="card">
                        <div class="card-header"><strong>Leave Type Form</strong></div>
							   ...
							   <div class="form-group">
								...
								<div class="form-group">
									...
									<select name="role" required class="form-control">
										...
										<?php
										$res=mysqli_query($con,"select * from role order by role desc ");
										while($row=mysqli_fetch_assoc($res)){
											if($role_id==$row['id']){
												echo "<option selected='selected' value=".$row['id'].">".$row['role']."</option>";
											}else{
